define (require) ->
    ...
    servicemapEngine = new Bloodhound
        name: 'suggestions'
        remote:
            url: apis.SERVICEMAP_BACKEND_BASE + "/search/?language=#{lang}&type=unit,service_node,street&page_size=4&input="
            replace: (url, query) =>
                url += query
    ...
    linkedeventsEngine = new Bloodhound
        ...
        remote:
            url: apis.LINKEDEVENTS_BACKEND_BASE + "/search/?type=event&language=#{lang}&page_size=4&input=%QUERY"
            ajax: settings.applyAjaxDefaults {}
            filter: (parsedResponse) ->
                parsedResponse.data
            rateLimitWait: 50
